...
impl PreviewEvaluationResult {
    ...
    fn update_summary(&mut self) {
        ...
        self.evaluation_summary.passed_materials = self.material_results
            .iter()
            .filter(|r| r.rule_evaluation.status_code == 200)
            .count();
        self.evaluation_summary.failed_materials = self.material_results
            .iter()
            .filter(|r| r.rule_evaluation.status_code != 200)
            .count();
        self.evaluation_summary.warning_materials = self.material_results
            .iter()
