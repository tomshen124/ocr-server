...
impl HealthChecker {
        ...
        match client.get(&health_url).send().await {
            Ok(response) => {
                if response.status().is_success() {
                    ...
                    Err(anyhow::anyhow!("健康检查API返回错误: {}", response.status()))
                }
            }
            Err(e) => {
                Err(anyhow::anyhow!("健康检查API调用失败: {}", e))
            }
        }
    }
    ...
    pub async fn check_components_health(&self) -> anyhow::Result<serde_json::Value> {
        ...
        let components_url = format!("http://127.0.0.1:{}/api/health/components", self.ocr_port);
        ...
        match client.get(&components_url).send().await {
            Ok(response) => {
                if response.status().is_success() {
                    ...
                    Err(anyhow::anyhow!("组件健康检查API返回错误: {}", response.status()))
                }
            }
            Err(e) => {
                Err(anyhow::anyhow!("组件健康检查API调用失败: {}", e))
            }
        }
    }
    ...
    pub async fn check_process_status(&self) -> anyhow::Result<bool> {
        ...
        for (_, process) in system.processes() {
            if process.name().contains("ocr-server") {
                debug!("找到OCR服务进程: PID={}, 名称={}", process.pid(), process.name());
                return Ok(true);
            }
        }
