...
pub fn monitoring_routes() -> Router<Arc<MonitorService>> {
    Router::new()
        ...
        .route("/api/monitoring/metrics", get(get_current_metrics))
        .route("/api/monitoring/history", get(get_metrics_history))
        .route("/api/monitoring/health", get(get_health_status))
        .route("/api/monitoring/ocr-status", get(get_ocr_status))
        .route("/api/monitoring/system-info", get(get_system_info))
}
...
pub async fn get_dashboard_data(
    ...
    let memory_trend: Vec<f32> = history.iter().rev().take(60).map(|m| m.memory_usage).collect();
