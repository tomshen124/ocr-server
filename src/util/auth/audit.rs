...
impl AccessLogger {
    ...
    pub fn get_access_statistics(&self, time_range: TimeRange) -> AccessStatistics {
        ...
        let successful_requests = filtered_records.iter()
        ...
        let avg_response_time = if successful_requests > 0 {
            let total_time: u64 = filtered_records.iter()
    ...
    pub fn get_security_statistics(&self, time_range: TimeRange) -> SecurityStatistics {
        ...
        let high_events = filtered_events.iter()
            ...
            .count();
        let unhandled_events = filtered_events.iter()
            .filter(|event| !event.handled)
            .count();

        let mut event_type_stats = HashMap::new();
        for event in &filtered_events {
            *event_type_stats.entry(format!("{:?}", event.event_type)).or_insert(0) += 1;
        }
    ...
    /// 获取指定客户端的访问历史
    pub fn get_client_access_history(
        &self,
        client_id: &str,
        limit: usize,
    ) -> Vec<AccessRecord> {
