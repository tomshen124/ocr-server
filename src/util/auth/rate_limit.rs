...
impl RateLimiter {
    ...
    pub async fn check_rate_limit(
        ...
        instance.check_client_rate_limit(client_id, hourly_limit).await
    }
    ...
    async fn check_client_rate_limit(
        ...
        // 获取或创建客户端记录
        let client_record = records.entry(client_id.to_string())
    ...
    fn cleanup_expired_records(&self, record: &mut ClientAccessRecord, now: DateTime<Utc>) {
        ...
        // 清理超过1小时的记录
        record.hour_requests.retain(|req| req.timestamp > one_hour_ago);
        
        // 清理超过1分钟的记录
        record.minute_requests.retain(|req| req.timestamp > one_minute_ago);
    ...
    pub async fn get_client_stats(&self, client_id: &str) -> ClientUsageStats {
        ...
        if let Some(record) = records.get(client_id) {
            ClientUsageStats {
                ...
                last_request_time: record.hour_requests.last()
...
impl SlidingWindowRateLimiter {
    ...
    pub fn check_limit(&self, client_id: &str) -> RateLimitResult {
        ...
        let window = windows.entry(client_id.to_string())
        ...
        // 清理窗口外的请求
        window.requests.retain(|&timestamp| timestamp > window_start);
        window.last_cleanup = now;

        // 检查是否超过限制
        if window.requests.len() >= self.max_requests as usize {
            let oldest_request = window.requests.first().cloned()
                .unwrap_or(now);
...
mod tests {
    ...
    async fn test_rate_limiter_basic() {
        ...
        for i in 0..hourly_limit {
            let result = limiter.check_client_rate_limit(client_id, hourly_limit).await;
