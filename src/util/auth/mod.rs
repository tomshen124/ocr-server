...
// 重新导出主要组件，保持向后兼容
pub use client::{ThirdPartyAuthService, AuthResult, AuthError, AuthenticatedClient, ApiAuthRequest};
pub use middleware::{third_party_auth_middleware, SecurityAuditor, RequestExt};
pub use rate_limit::{RateLimiter, ClientUsageStats, SlidingWindowRateLimiter, TokenBucketRateLimiter};
pub use audit::{AccessLogger, AccessRecord, SecurityEvent, SecurityEventType, SecuritySeverity, TimeRange};
...
impl ThirdPartyAuthManager {
    ...
    pub async fn handle_auth_request(
        ...
        // 提取认证信息
        let auth_info = ThirdPartyAuthService::extract_auth_info(headers, params)
            .map_err(|error| {
        ...
        let authenticated_client = match auth_result {
            ...
            AuthResult::Failed(error) => {
                let access_result = match error {
                    AuthError::InvalidClient(_) => audit::AccessResult::AuthFailed(error.message().to_string()),
                    AuthError::InsufficientPermission(_) => audit::AccessResult::PermissionDenied(error.message().to_string()),
                    AuthError::RateLimitExceeded(_) => audit::AccessResult::RateLimited(error.message().to_string()),
...
pub fn requires_third_party_auth(path: &str) -> bool {
    ...
    protected_patterns.iter().any(|pattern| path.starts_with(pattern))
}
...
mod tests {
    ...
    async fn test_auth_manager_creation() {
        ...
        // 验证组件已正确初始化
        let stats = manager.access_logger().get_access_statistics(TimeRange::LastHour);
