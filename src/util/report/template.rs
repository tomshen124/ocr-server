...
impl TemplateEngine {
    ...
    fn load_default_templates(&mut self) {
        ...
        self.templates.insert(
            ...
            Self::get_standard_template(),
        );

        // 简化预览模板
        self.templates.insert(
            "simple_preview".to_string(),
            Self::get_simple_template(),
        );
        ...
        json!({
            ...
            "materials": result.material_results.iter().enumerate().map(|(index, material)| {
                json!({
                    ...
                    "status_icon": if material.rule_evaluation.status_code == 200 { "✅" } else { "❌" },
    ...
    fn simple_template_replace(&self, template: &str, data: &Value) -> String {
        ...
        if let Some(basic_info) = data.get("basic_info") {
            result = result.replace("{{applicant_name}}", basic_info["applicant_name"].as_str().unwrap_or(""));
            result = result.replace("{{agent_name}}", basic_info["agent_name"].as_str().unwrap_or(""));
            result = result.replace("{{matter_name}}", basic_info["matter_name"].as_str().unwrap_or(""));
            result = result.replace("{{matter_type}}", basic_info["matter_type"].as_str().unwrap_or(""));
            result = result.replace("{{request_id}}", basic_info["request_id"].as_str().unwrap_or(""));
            result = result.replace("{{sequence_no}}", basic_info["sequence_no"].as_str().unwrap_or(""));
        }

        // 替换评估摘要变量
        if let Some(summary) = data.get("evaluation_summary") {
            result = result.replace("{{total_materials}}", &summary["total_materials"].to_string());
            result = result.replace("{{passed_materials}}", &summary["passed_materials"].to_string());
            result = result.replace("{{failed_materials}}", &summary["failed_materials"].to_string());
            result = result.replace("{{overall_result}}", summary["overall_result"].as_str().unwrap_or(""));
            result = result.replace("{{overall_result_class}}", summary["overall_result_class"].as_str().unwrap_or(""));
        }

        // 替换时间变量
        result = result.replace("{{evaluation_time}}", data["evaluation_time"].as_str().unwrap_or(""));
        result = result.replace("{{current_time}}", data["current_time"].as_str().unwrap_or(""));
